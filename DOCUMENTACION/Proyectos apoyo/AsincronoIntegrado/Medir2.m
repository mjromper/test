...
% Estadísticos
global num_vueltas t_total t_bloque num_eleccx t_invocaxRed t_invocaxGreen sujeto sesion t_pause tiempo_en_bloq 
global commandTime commandTimeTot selectedOption commands explore;
...
%2- EMPIEZA A MUESTREAR
   ...
   % hacemos la media por cada comando (independientemente de se fue un acierto o no)
   totalEleccx = num_eleccx(1) + num_eleccx(2) + num_eleccx(3);
   t_invocax = t_invocaxGreen + t_invocaxRed;
   t_invocaxMedia = sum(t_invocax) / totalEleccx;
   t_totalMedia = ( t_total(1) + t_total(2) + t_total(3) ) / totalEleccx;      
   t_bloqueMedia = ( t_bloque(1) + t_bloque(2) + t_bloque(3) ) / totalEleccx;
   ...
   succes = [0 0 0];
   t_bloque_acierto = [0 0 0];
   t_total_acierto = [0 0 0];
   ...
   comando1 = ['save ' 'ESTADISTICAS' '\estadisticos_' sujeto '_' num2str(sesion) '.mat num_vueltas t_total ' ...
       ...
       't_bloque_acierto_media t_total_acierto_media t_invocaxMedia t_invocaxGreen t_invocaxRed'];
   eval(comando1);         
