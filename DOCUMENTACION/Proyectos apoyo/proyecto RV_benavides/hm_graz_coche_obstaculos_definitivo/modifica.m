CAMBIOS IMPORTANTES
    ...
    -Prueba: 
     ...
     El programa original HM siempre procesaba las 4 señales, y para ello llamaba a la funcion "tiempo2frec.m".
    ...
    if num_canales_procesar==4, tiempo2frec_4cb(muestra1,muestra2,muestra3,muestra4,inicio); end; %NV PROCESO EL CANAL UNO, DOS, TRES Y CUATRO
     ...
     <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
      ...
      - En el programa HM-panel, he moificado la linea:
        ...
        - Un problema en el plot de las señales EEG era que las diferentes tramas no se conectaban, por lo que
        ...
        - Voy a introducir un canal mas pero solo va a ser para capturar la señal y no para procesar. Esto va a afectar 
