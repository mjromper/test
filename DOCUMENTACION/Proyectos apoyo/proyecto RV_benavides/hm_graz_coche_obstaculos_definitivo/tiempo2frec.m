...
function tiempo2frec(m1,m2,m3,m4,ini)
...
%1- Preparación de Variables y Parámetros:
   ...
   if solapef<primera_muestra, %Cogemos el 'solapef' completo:
      ...
      s3=m3(primera_muestra-solapef : ultima_muestra);
      s4=m4(primera_muestra-solapef : ultima_muestra);
      ...
      s3=m3(1 : ultima_muestra);
      s4=m4(1 : ultima_muestra);
   end;
   %length(s1)
...
%2- Filtro las muestras:
   ...
   while Numtotal3(1)==0, Numtotal3 = Numtotal3(2:end);	end;
   while Dentotal3(1)==0, Dentotal3 = Dentotal3(2:end);	end;
   while Numtotal4(1)==0, Numtotal4 = Numtotal4(2:end);	end;
   while Dentotal4(1)==0, Dentotal4 = Dentotal4(2:end);	end;
   ...
   s3= filter(Numtotal3,Dentotal3,s3); %Filtrado propiamente dicho del canal 3
   s4= filter(Numtotal4,Dentotal4,s4); %Filtrado propiamente dicho del canal 4
...
%2'5- Quito el 'solapef':
   if solapef<=primera_muestra, %Se pudo coger el 'solapef' completo, ahora se quita:
      ...
      s3= s3(solapef+1 : solapef+solape+tam);
      s4= s4(solapef+1 : solapef+solape+tam);
      ...
      s3= s3(primera_muestra : ultima_muestra);
      s4= s4(primera_muestra : ultima_muestra);
   end;
...
%3- En 'fini' almaceno todas las muestras enventanadas desde que he llamado 
    ...
    fin1(indice*tam2+1:(indice+1)*tam2)= s1;
    fin2(indice*tam2+1:(indice+1)*tam2)= s2;
    fin3(indice*tam2+1:(indice+1)*tam2)= s3;
    fin4(indice*tam2+1:(indice+1)*tam2)= s4;
...
%4- Calculo la Estimacion Espectral para 'tam2=tam+solape' muestras:
   ...
   case 1,%La FFT no es una estimacion de la DEP, sino de la DFT.
      ...
      P3= abs(fft(s3,nump))/length(s3);%Para que no dependa de la longitud de s3.
      P3= P3(1:floor(nump/2)+1);%Nos quedamos solo las frecuencias '0' a 'Fs/2'
      P3= 2*P3; %Estamos dando todos los resultados 'onesided' osea con toda la potencia...
      P3(1)= P3(1)/2; %(frecuencia 0Hz)Leer arriba
      if rem(nump,2)==0,   P3(end)= P3(end)/2; end; %Si no es cero(nump impar, ...
      P4= abs(fft(s4,nump))/length(s4);%Para que no dependa de la longitud de s4.
      P4= P4(1:floor(nump/2)+1);%Nos quedamos solo las frecuencias '0' a 'Fs/2'
      P4= 2*P4; %Estamos dando todos los resultados 'onesided' osea con toda la potencia...
      P4(1)= P4(1)/2; %(frecuencia 0Hz)Leer arriba
      if rem(nump,2)==0,   P4(end)= P4(end)/2; end; %Si no es cero(nump impar, ...
   case 2,
      ...
      P3= periodogram(s3,ones(size(s3)),nump,Fs,'onesided')/(length(s3)/Fs);%Dividir por el nº de segundos
      P4= periodogram(s4,ones(size(s4)),nump,Fs,'onesided')/(length(s4)/Fs);%Dividir por el nº de segundos      
   case 3,%CUIDADO: pburg devuelve la potencia escalada segun la frecuencia que se le da:
      ...
      P3= pburg(s3,orden,nump,Fs,'onesided')*(Fs/2);
      P4= pburg(s4,orden,nump,Fs,'onesided')*(Fs/2);
   case 4,
      %Tiene el mismo problema que Burg:
      P1= pmcov(s1,orden,nump,Fs,'onesided')*(Fs/2);
      P2= pmcov(s2,orden,nump,Fs,'onesided')*(Fs/2);
      P3= pmcov(s3,orden,nump,Fs,'onesided')*(Fs/2);
      P4= pmcov(s4,orden,nump,Fs,'onesided')*(Fs/2);
   case 5,%AHORA MISMO NO ESTA ACTIVADO COMO OPCION POSIBLE
      ...
      ventana='kavehlippert';
      P1= pburgw(s1,orden,nump,ventana,Fs);%Funciona, solo queda comprobar q se introducen bien los parametros en el panel de control
      P2= pburgw(s2,orden,nump,ventana,Fs);
      P3= pburgw(s3,orden,nump,ventana,Fs);
      P4= pburgw(s4,orden,nump,ventana,Fs);
   end;
   ...
   if ver_f==1,
      ...
      if grabar_c1,   plot(f,P1y,'Color',COLOR_C1);     end;
      if grabar_c2,   plot(f,P2y,'Color',COLOR_C2);     end;
      if grabar_c3,   plot(f,P3y,'Color',COLOR_C3);     end;
      if grabar_c4,   plot(f,P4y,'Color',COLOR_C4);     end;       
      hold off
   end;    
   figure(fig_sujeto); %Hay que volver a figure(fig_sujeto)
...
%5- En "final1" y "final2" almaceno el resultado de la BURG de todas las ventanas
    ...
    final1(indice*nump2+1:(indice+1)*nump2)=P1y;
    final2(indice*nump2+1:(indice+1)*nump2)=P2y; 
    final3(indice*nump2+1:(indice+1)*nump2)=P3y;
    final4(indice*nump2+1:(indice+1)*nump2)=P4y;
    
    %En finaltotal almaceno tdas las respuestas en frec de toda la sesion
    inicialtotal=indicetotalb*nump2+1;
    finaltotal1(inicialtotal:(indicetotalb+1)*nump2)=P1y; 
    finaltotal2(inicialtotal:(indicetotalb+1)*nump2)=P2y;
    finaltotal3(inicialtotal:(indicetotalb+1)*nump2)=P3y; 
    finaltotal4(inicialtotal:(indicetotalb+1)*nump2)=P4y;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   if (n_de_bloque == n_tot_bloq_por_prueba) | acierto, %Cuando se han capturado todos los bloques de una prueba,
      %(Cuando hay acierto se graba lo que se tenga en ese momento).
      
  
   
      %Grabo la prueba completa:
      comando=['save ' trayectoria '\rfb' int2str(pruebactual) '.mat sujeto ensayo Fs tam_ventana orden'];
      if grabar_c1, comando=[comando ' final1 c1']; end
      if grabar_c2, comando=[comando ' final2 c2']; end
      if grabar_c3, comando=[comando ' final3 c3']; end
      if grabar_c4, comando=[comando ' final4 c4']; end
      tipo=str2num(tipologia(pruebactual)); %Lo guardare como un numero
      comando=[comando ' f tipo'];
      eval(comando); %Grabamos un fichero 'rti.mat' por cada prueba
      disp(['prueba.m: ' num2str(ntot_pru)]) %La sabe pero dice que no
      
      if pruebactual==ntot_pru, 
          
               %NV QUITAR
 %  save basura.mat s1 s2 s3 s4 Numtotal1 Numtotal2 Numtotal3 Numtotal4 Dentotal1 Dentotal2 Dentotal3 Dentotal4
save basura.mat finaltotal1 finaltotal2 finaltotal3 finaltotal4
   %pause
   %break  
         %Grabamos un fichero rtotal.m conteniendo todas las pruebas juntas:
         comando=['save ' trayectoria '\rfbtotal.mat sujeto ensayo Fs tam_ventana orden'];
         if grabar_c1, comando=[comando ' finaltotal1']; end
         if grabar_c2, comando=[comando ' finaltotal2']; end
         if grabar_c3, comando=[comando ' finaltotal3']; end
         if grabar_c4, comando=[comando ' finaltotal4']; end
         comando=[comando ' f tipologia'];
         eval(comando);
      end %if
...
n_tot_bloq_por_prueba= duration * Fs/tam; %duration=(duracion de la prueba: se define en Ensayo)
if indice==n_tot_bloq_por_prueba;  %Son 40 bloques los que voy a obtener(4 segundos a 0.1s cada bloque)
     indice=0;
     %tiniburg=0;
end
