...
global perm_plot_prc1 %NV PARA DAR PERMISO A DIBUJAR LOS PLOTS FILTADOS

persistent  ti_filt s1_filt %NV VARIABLES PARA EL PLOT DE LAS TRAZAS FILTRADAS  cont_filt


persistent  indice final1 final2 final3 final4 fin1 fin2 vmax frecmax tiniburg indicetotalb 
persistent ini_anterior P1_anterior  cont_filt 
...
%1- Preparación de Variables y Parámetros:
   ...
   if solapef<primera_muestra, %Cogemos el 'solapef' completo:
      ...
      %NV s2=m2(primera_muestra-solapef : ultima_muestra);
...
%2- Filtro las muestras:
   ...
   %NVwhile Numtotal2(1)==0, Numtotal2 = Numtotal2(2:end);	end;
   %NVwhile Dentotal2(1)==0, Dentotal2 = Dentotal2(2:end);	end;
   ...
   %NVs2= filter(Numtotal2,Dentotal2,s2); %Filtrado propiamente dicho del canal 2  
...
%2'5- Quito el 'solapef':
   if solapef<=primera_muestra, %Se pudo coger el 'solapef' completo, ahora se quita:
     ...
     %NV s2= s2(solapef+1 : solapef+solape+tam);
     ...
     %NV s2= s2(primera_muestra : ultima_muestra);
...
%4- Calculo la Estimacion Espectral para 'tam2=tam+solape' muestras:
   ...
   case 1,%La FFT no es una estimacion de la DEP, sino de la DFT.
      ...
      %NVP2= abs(fft(s2,nump))/length(s2);%Para que no dependa de la longitud de s2.
      %NVP2= P2(1:floor(nump/2)+1);%Nos quedamos solo las frecuencias '0' a 'Fs/2'
      %NVP2= 2*P2; %Estamos dando todos los resultados 'onesided' osea con toda la potencia...
      %NVP2(1)= P2(1)/2; %(frecuencia 0Hz)Leer arriba
      %NVif rem(nump,2)==0,   P2(end)= P2(end)/2; end; %Si no es cero(nump impar, ...
   ...
   case 2,
      ...
      %NVP2= periodogram(s2,ones(size(s2)),nump,Fs,'onesided')/(length(s2)/Fs);%Dividir por el nº de segundos
   ...
   case 3,%CUIDADO: pburg devuelve la potencia escalada segun la frecuencia que se le da:
      ...
      %NVP2= pburg(s2,orden,nump,Fs,'onesided')*(Fs/2);
   ...
   case 4,
      ...
      %NVP2= pmcov(s2,orden,nump,Fs,'onesided')*(Fs/2);
...
%6- La funcion descursor va a desplazar el cursor. Ademas me va a calcular
    ...
    if str2num(tipologia(pruebactual))==6, %(Solo se llama a Descursor si la prueba es tipo JUGAR)
       ...
       if tiempo1>t_cursor,
          ...
          %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%NV
          % VOY A CREAR UNA FUNCION QUE DESPLAZE EL MUNDO VIRTUAL
