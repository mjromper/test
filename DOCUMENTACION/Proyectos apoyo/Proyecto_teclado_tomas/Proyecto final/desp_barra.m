...
%   una selección o no, y ejecuta la llamada a la siguiente función dependiendo 
%   de el suceso anterior (selección o desplazamiento)
%
%
...
if dist>0                   % Si la dist es positiva, el sujeto está realizando la tarea de seleccionar.  
    ...
    bar=long_minima;        % si se está realizando el otro estado mental, la barra tendrá una longitud minima(0,35)
end;

%/----------------------------------------------------------------------------------------

if bar<=0.35            % si el tamaño de barra (dist) es minimo, la barra tiene q medir una long minima (0,35)
    bar=long_minima;        
    t_bloq=0;
elseif bar>0.7          % la "dist" supera el umbral del bloque, inicio el contador    
    ...
    if bar>1            % compruebo si el tamaño de la barra es el máximo, es decir sobrepasa la circunferencia exterior
        bar=long_max;   % si la "dist" es más grande de la cuenta, pongo una long máxima para el tamaño de la flecha
    end,
    mueve=mueve+vel;    % si entro en el limite de la rueda, paro la flecha para poder seleccionarla bien. 
                        % Para parar el desplazamiento, se suma la velocidad para anular la resta que se hará posteriormente
else                    
...
if t_bloq==tiempo_en_bloq,   % si he estado el suficiente tiempo en el bloque, lo selecciono
    ...
    if a_rueda==0,      % cambio el color de la barra para destacar que ha habido un selección.
...
elseif (selec==1) & (a_rueda==0)        % voy a seleccionar el bloque en la rueda pral
    ...
    mueve=90-vel;                       % para desplazar la barra en la rueda auxiliar empiezo en 90º(menos la velocidad)
...
elseif (selec==1) & (a_rueda==1)        % selecciono la letra en la rueda auxiliar
    ...
    selec=0;                            % reinicio selección para la proxima vez
    mueve=90-vel;                       % prepara la barra en la posición de 90º menos la velocidad elegida
    tecla=1;                            % reinicio también la variable tecla, que indica el bloque al que voy                
