...
persistent ini_anterior P1_anterior  P2_anterior P3_anterior P4_anterior%NV


global ver_t t fig_tiempo COLOR_C1 COLOR_C2 COLOR_C3 COLOR_C4 %NV PARA DIBUJAR LOS PLOTS FILTRADOS
global perm_plot_prc1 perm_plot_prc2 perm_plot_prc3 perm_plot_prc4%NV PARA DAR PERMISO A DIBUJAR LOS PLOTS FILTADOS

persistent cont_filt ti_filt s1_filt s2_filt s3_filt s4_filt%NV VARIABLES PARA EL PLOT DE LAS TRAZAS FILTRADAS
...
%NV AHORA EMPIEZO A DIBUJAR SI HE ACTIVADO LA OPCION DE DIBUJAR
    ...
    ss3=s3((length(s3)-tam+1):length(s3));
    ss4=s4((length(s4)-tam+1):length(s4));
    if cont_filt==0  %NV CON ESTO DIBUJO LA PRIMERA TRAZA TAL CUAL
       ...
       if perm_plot_prc3, plot(t,ss3,'Color',COLOR_C3,'EraseMode','none');   end;
       if perm_plot_prc4, plot(t,ss4,'Color',COLOR_C4,'EraseMode','none');   end;
    cont_filt=1;  %NV CAMBIO LA VARIABLE cont_filt PARA QUE EN LA SIGUIENTES TRAZAS DIBUJE CONECTANTO
        ...
        tt=t';
          ...
          if perm_plot_prc3, plot([ti_filt,tt],[s3_filt,ss3],'Color',COLOR_C3,'EraseMode','none');  end;
          if perm_plot_prc4, plot([ti_filt,tt],[s4_filt,ss4],'Color',COLOR_C4,'EraseMode','none');  end;
    end; 
...
%4- Calculo la Estimacion Espectral para 'tam2=tam+solape' muestras:
   ...
   case 5,%AHORA MISMO NO ESTA ACTIVADO COMO OPCION POSIBLE
      ...
      P3=(sum(s3.*s3))/length(s3);
      vector_pot2(ultima_muestra)=P3;
      
      P4=(sum(s4.*s4))/length(s4);
      vector_pot2(ultima_muestra)=P4;
   ...
   if (ver_f==1) 
      ...
      plot(f,P3y,'Color',COLOR_C3);    %NV
      plot(f,P4y,'Color',COLOR_C4);    %NV
      hold off
         ...
         plot([(ini_anterior/Fs) (ultima_muestra /Fs)],[P3_anterior P3],'Color',COLOR_C3); 
         plot([(ini_anterior/Fs) (ultima_muestra /Fs)],[P4_anterior P4],'Color',COLOR_C4); 
         hold on;
...
%7- Calcula el valor maximo de BURG en cada ventana y su frecuencia.
    ...
    %NV tiempoB=toc-tiniburg(indice+1)
